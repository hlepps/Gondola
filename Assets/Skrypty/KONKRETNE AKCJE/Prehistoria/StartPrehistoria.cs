...
public class StartPrehistoria : MonoBehaviour {
	...
	void Update () {
        if (action)
        {
            GameObject.FindGameObjectWithTag("Global").GetComponent<Global>().waiting = true;
            ...
            if(kermit.GetComponent<Kot>().ResetAction(4))
            {
                quest.GetComponent<Quest>().newQuest("Znajdź kota");
                GameObject.FindGameObjectWithTag("Global").GetComponent<Global>().waiting = false;
                action = false;
            }
        }
